<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right" style="padding-left: 10px;padding-bottom: 10px;font-size: 12px">
      ...
      <el-breadcrumb-item>{{ this.$route.query.sort.sortName }}</el-breadcrumb-item>
    </el-breadcrumb>
    <!--视频分类卡片-->
...
import { findSecondSortList, addNewSecondSort, deleteSecondSort } from '../../api/system'
...
export default {
  ...
  methods: {
    ...
    async getSecondSortList () {
      const { data } = await findSecondSortList(this.current, this.pageSize, this.$route.query.sort.sortName)
      this.sortList = data.data.SortLevelTwo
    ...
    addSort () {
      this.$refs.addRuleForm.validate(async valid => {
        ...
        const { data } = await addNewSecondSort(this.addForm.sortname, this.$route.query.sort.sortName)
        console.log(data.data)
        this.getSecondSortList()
        this.$message.success('已成功添加子分类<' + data.data.sort.sortName + '>到分类<' + this.$route.query.sort.sortName + '>中')
    ...
    async removeSortById (sortName) {
      ...
      // eslint-disable-next-line no-unused-vars
      const { data } = await deleteSecondSort(sortName, this.$route.query.sort.sortName)
      console.log(data)
      if (data.code !== 200) {
    ...
    handleTaskItemClick (e) {
      ...
      this.$router.push({ path: '/sortData/' + e.sortId + '/SecondLevel', query: { name: e.sortId } })
    }
  }
}
</script>
